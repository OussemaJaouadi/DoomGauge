...
### Live tracking implementation — September 2026
YouTube Shorts, Instagram Reels and Facebook Reels now have content entrypoints feeding one focused-window visit tracker. Visits checkpoint into IndexedDB with retry-safe identities and revisions. Normal exits complete visits; abandoned visits recover as interrupted without inferred time. Popup and telemetry use recorded observations by default, including saved unfinished time, and expose loading/read failures and saving interruptions. Records identify completed, in-progress and interrupted visits. Stop-loss and optional platform mechanics remain unimplemented.

Coverage is deliberately conservative: verified foreground collector intervals only. Browser-off time, other apps and unsupported/unrecognized views remain unknown. Complete-day recurrence and covered return follow-up can therefore remain unavailable; an empty recorded interval is not proof of zero usage. Real-site selector validation remains a manual acceptance step. Existing open platform tabs need refreshing after loading/updating the extension. Development fixtures remain available with `?data=mock` or a non-normal data preset; production never uses them.
