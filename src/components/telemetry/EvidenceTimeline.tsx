...
export function EvidenceTimeline({ sessions, events, window, selectedId, onSelect, date }: { sessions: ObservationSession[]; events: PreviewObservation[]; window?: RecurringWindow; selectedId: string | null; onSelect: (id: string) => void; date?: string }) {
  ...
  return <StateRegion state={contentState(rows.length)} id="evidence.intervals" label="Session intervals" shape="chart"><section className="evidence-visual">
...
export function ReturnTimeline({ matches, selectedId, onSelect }: { matches: SessionReturn[]; selectedId: string | null; onSelect: (id: string, unfiltered: boolean) => void }) {
  ...
  return <StateRegion state={contentState(matches.length)} id="evidence.pairs" label="Return pairs" shape="chart" reasons={["followup", "activity", "unobserved"]}><section className="evidence-visual"><div className="evidence-visual-heading"><h3>Origin and return</h3><Hint label="About return pairs" text={measurementHints.pairs} accent="blue" /></div>
