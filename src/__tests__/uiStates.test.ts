...
import { resolveState, retryOverrides, emptyMessages, type SkeletonShape } from '../components/ui/stateModel';
...
test('loading hides real values and exposes one named status with decorative shapes', () => {
  const html = renderToStaticMarkup(createElement(StateRegion, { id: 'test', label: 'Reel count', shape: 'metric', actual: { status: 'loading' }, children: '428 reels' }));
...
test('local errors hide only their own values and include recovery', () => {
  const html = renderToStaticMarkup(createElement(Fragment, null,
    createElement(StateRegion, { id: 'broken', label: 'Activity', actual: { status: 'error' }, children: 'stale value' }),
    createElement(StateRegion, { id: 'healthy', label: 'Reels', children: '19 reels' }),
  ));
...
test('filter empty states expose scoped recovery and settings restore only mock defaults', () => {
  const html = renderToStaticMarkup(createElement(StateRegion, { id: 'filtered', label: 'Records', actual: { status: 'empty', reason: 'filters' }, onClearFilters: () => {}, children: 'hidden' }));
  expect(html.includes('No matching activity')).toBe(true);
  expect(html.includes('Clear filters')).toBe(true);
  const settings = renderToStaticMarkup(createElement(StateRegion, { id: 'settings', label: 'Settings', reasons: ['settings'], actual: { status: 'empty', reason: 'settings' }, children: 'hidden' }));
  expect(settings.includes('Restore mock defaults')).toBe(true);
  expect(new Set(Object.values(emptyMessages).map(message => message.title)).size).toBe(7);
});
test('data fixtures reconcile zero, sparse history and previous-only comparisons', () => {
  ...
  expect(normal.mock.totalCount > 0).toBe(true);
  expect(zero.mock.totalCount).toBe(0);
  expect(zero.mock.yesterdayCount).toBe(0);
  expect(zero.distribution.medianMs).toBe(null);
  expect(zero.hourly.reduce((sum, hour) => sum + hour.total, 0)).toBe(0);
  expect(sparse.mock.totalCount).toBe(1);
  expect(old.mock.totalCount).toBe(0);
  expect(old.mock.yesterdayCount > 0).toBe(true);
  expect(popupStateFixture('filtered').mock.totalCount).toBe(0);
});
...
test('preview controls are absent outside a development build', () => {
  expect(renderToStaticMarkup(createElement(StatePreviewControls))).toBe('');
});
