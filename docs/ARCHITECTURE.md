...
### Live tracker and recovery
Three WXT content entrypoints share `src/tracking/`: platform detection, a pure visit state machine, cumulative outbox, validated background ingestion, IndexedDB adapter and UI queries. DOM/intersection observers and media events detect transitions; a 500ms sampling loop provides timing and SPA-route fallback with monotonic elapsed measurements, not fixed tick increments. Media events pause promptly; focus requires both document focus and a short-lived background confirmation of the active tab in the focused window. Buffering/seeking/hidden media contributes no active time. Long sampling gaps or clock discontinuities interrupt at the last sampled timestamp.

Visits are keyed by random ID and bound to collector/document/tab identity. Start, state changes and approximately two-second foreground checkpoints upsert cumulative intervals; older revisions do not overwrite newer ones and recorded interval prefixes cannot change. Strict-durability readwrite transactions acknowledge only on completion. Unacknowledged snapshots remain in the content outbox; closing that page can lose its unsaved tail. No absolute crash/power-loss guarantee is implied.

Additive stores in `doomgauge-v1`: `visits` (ID key; end/status indexes), `trackingCoverage` (ID key/end index), `trackingMeta` (dirty date keys), and `rollups` (date/platform key). Preserve preferences and any legacy events. Only the background accesses the database. `tracking:visit`, `tracking:heartbeat` and `tracking:health` require main-frame senders on supported hosts; `tracking:query` requires an extension page and a bounded date range. The background probes surviving collectors on startup/minute maintenance; abandoned open visits older than the grace period become interrupted at their last observation. Dirty rollups rebuild transactionally on maintenance or query, so missed midnight alarms do not lose summaries.

Root visits adapt to the existing rendering contracts with optional active intervals/status/count-in-scope fields. Selection clips recorded time to calendar/daypart windows without changing identity; count belongs to the start. Unknown coverage yields gaps in trend zeroes, unavailable comparisons, and excluded recurrence/return denominators. The live UI polls only while visible and retries failed reads. Saving failures are shown in the UI and extension badge. Raw source metadata contains reel identity, not full URLs or page text. No tracking permissions were added; fake-indexeddb is development-only transaction-test infrastructure.
